...
public class TableOCs<OC extends OperationChain> {
    public ConcurrentHashMap<Integer, Holder<OC>> threadOCsMap = new ConcurrentHashMap<>();//each op has a holder.

    public TableOCs(Integer num_op) {
        int i;
        for (i = 0; i < num_op; i++) {
            threadOCsMap.put(i, new Holder<OC>());
        }
    }
}
