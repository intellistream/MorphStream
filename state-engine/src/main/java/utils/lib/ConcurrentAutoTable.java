...
public class ConcurrentAutoTable implements Serializable {
     ...
     * total counter value_list (i.e., {@link #get}) is not atomically updated.
    ...
    /**
     * Atomically set the sum of the striped counters to specified value_list.
    ...
    /**
     * Return the counter's {@code long} value_list converted to a string.
     */
    ...
    // value_list - which WILL have zero under the mask on success and WILL NOT have
    ...
    private static class CAT implements Serializable {
        ...
        // Fast fuzzy version.  Used a cached value_list until it gets old, then re-up
        // the cache.
        public long estimate_sum(long mask) {
            // For short tables, just do the work
            if (_t.length <= 64) return sum(mask);
            // For bigger tables, periodically freshen a cached value_list
            long millis = System.currentTimeMillis();
            if (_fuzzy_time != millis) { // Time marches on?
                _fuzzy_sum_cache = sum(mask); // Get sum the hard way
                _fuzzy_time = millis;   // Indicate freshness of cached value_list
            }
