 ...
 * does <em>not</em> allow <tt>null</tt> to be used as a key or value_list.
 *
...
public class NonBlockingHashMap<TypeK, TypeV>
    ...
    private static final Object get_impl(final NonBlockingHashMap topmap, final Object[] kvs, final Object key, final int fullhash) {
        ...
        while (true) {
            ...
            final Object V = val(kvs, idx); // Get value_list before volatile read, could be null or Tombstone or Prime
            ...
            if (keyeq(K, key, hashes, idx, fullhash)) {
                ...
                if (!(V instanceof Prime)) // No copy?
                    return (V == TOMBSTONE) ? null : V; // Return the value_list
    ...
    // the path through copy_slot passes in an expected value_list of null, and
    ...
    private static final Object putIfMatch(final NonBlockingHashMap topmap, final Object[] kvs, final Object key, final Object putval, final Object expVal) {
        ...
        while (true) {             // Spin till we get a Key slot
            V = val(kvs, idx);         // Get old value_list (before volatile read below!)
            ...
            if (K == null) {         // Slot is free?
                ...
                K = key(kvs, idx);       // CAS failed, get updated value_list
        ...
        while (true) {
            ...
            // Else CAS failed
            V = val(kvs, idx);         // Get new value_list
     ...
     * <p> The value_list can be retrieved by calling {@link #get} with a key that is
     ...
     * @throws NullPointerException if the specified key or value_list is null
     */
    @Override
     ...
     * <tt>", "</tt> (comma and space).  Each key-value_list mapping is rendered as
     * the key followed by an equals sign (<tt>"="</tt>) followed by the
     * associated value_list.  Keys and values are converted to strings as by
     ...
     * a value_list {@code v} such that {@code key.equals(k)}, then this method
        ...
        private boolean copy_slot(NonBlockingHashMap topmap, int idx, Object[] oldkvs, Object[] newkvs) {
            ...
            while (!(oldval instanceof Prime)) {
                ...
                if (CAS_val(oldkvs, idx, oldval, box)) { // CAS down a box'd version of oldval
                    ...
                    // return with true here: any thread looking for a value_list for
