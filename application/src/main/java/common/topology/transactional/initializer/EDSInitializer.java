...
import benchmark.datagenerator.apps.EDS.TPGTxnGenerator.EDSTPGDataGenerator;
import benchmark.datagenerator.apps.EDS.TPGTxnGenerator.EDSTPGDataGeneratorConfig;
import benchmark.datagenerator.apps.EDS.TPGTxnGenerator.EDSTPGDynamicDataGenerator;
...
public class EDSInitializer extends TableInitilizer {
    private static final Logger LOG = LoggerFactory.getLogger(EDSInitializer.class);
    ...
    public EDSInitializer(Database db, int numberOfStates, double theta, int tthread, Configuration config) {
    ...
    protected void createTPGGenerator(Configuration config) {
        if (config.getBoolean("isDynamic")) {
            ...
            dataGenerator = new EDSTPGDynamicDataGenerator(dynamicDataGeneratorConfig);
        } else {
            EDSTPGDataGeneratorConfig dataConfig = new EDSTPGDataGeneratorConfig();
            dataConfig.initialize(config);
            configurePath(dataConfig);
            dataGenerator = new EDSTPGDataGenerator(dataConfig);
        }
    }
    ...
    private void configurePath(DataGeneratorConfig dataConfig) {
        ...
        try {
            ...
            byte[] bytes;
            if (dataConfig instanceof EDSTPGDataGeneratorConfig)
...
//        int p_bids[] = new int[tthread];
    ...
    public void store(String file_name) throws IOException {
        ...
        for (Object event : db.getEventManager().input_events) {
            TRSEvent trEvent = (TRSEvent) event;
            String sb =
