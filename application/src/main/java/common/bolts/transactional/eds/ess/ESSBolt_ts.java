...
public class ESSBolt_ts extends ESSBolt {
    private static final Logger LOG = LoggerFactory.getLogger(ESSBolt_ts.class);
    private static final long serialVersionUID = -5968750340131744744L;
    //write-compute time pre-measured.
    ArrayDeque<ESSEvent> esEvents;
    private double windowBoundary;
    ArrayDeque<Tuple> outWindowEvents;

    //To be used in Combo
    public ESSBolt_ts(int fid, SINKCombo sink) {
        super(LOG, fid, sink);

    }

    //To be used in ED Topology
    public ESSBolt_ts(int fid) {
        super(LOG, fid, null);
    }
    ...
    protected void PRE_TXN_PROCESS(double _bid, long timestamp) throws DatabaseException, InterruptedException {
        ...
        for (double i = _bid; i < _bid + combo_bid_size; i++) {
            ...
            ESSEvent event = (ESSEvent) input_event;
    ...
    protected void EVENT_SELECT_REQUEST_CONSTRUCT(ESSEvent event, TxnContext txnContext) throws DatabaseException {
...
//            LOG.info("Thread " + this.thread_Id + " has reached punctuation: " + windowBoundary);
            ...
            while (!outWindowEvents.isEmpty()) {
                ...
                if (outWindowTuple.getBID() >= total_events) {//if the out-of-window events are stopping signals, directly pass to downstream
                    EVENT_SELECT_REQUEST_POST((ESSEvent) outWindowTuple.getValue(0));
