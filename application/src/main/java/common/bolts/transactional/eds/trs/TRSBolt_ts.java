...
public class TRSBolt_ts extends TRSBolt {
    private static final Logger LOG = LoggerFactory.getLogger(TRSBolt_ts.class);
    private static final long serialVersionUID = -5968750340131744744L;
    //write-compute time pre-measured.
    ArrayDeque<TRSEvent> trEvents;
    private double windowBoundary;

    //To be used in Combo
    public TRSBolt_ts(int fid, SINKCombo sink) {
        super(LOG, fid, sink);
    }

    //To be used in ED Topology
    public TRSBolt_ts(int fid) {
        super(LOG, fid, null);
    }
    ...
    protected void PRE_TXN_PROCESS(double _bid, long timestamp) throws DatabaseException, InterruptedException {
        ...
        for (double i = _bid; i < _bid + combo_bid_size; i++) {
            ...
            TRSEvent event = (TRSEvent) input_event;
    ...
    protected void TWEET_REGISTRANT_REQUEST_CONSTRUCT(TRSEvent event, TxnContext txnContext) throws DatabaseException, InterruptedException {
...
//            LOG.info("Thread " + this.thread_Id + " increment window boundary to: " + windowBoundary);
            ...
            if (bid >= total_events) {
                EMIT_STOP_SIGNAL((TRSEvent) in.getValue(0));
                // Stop itself
