...
public class TCSBolt_ts extends TCSBolt {
    private static final Logger LOG= LoggerFactory.getLogger(TCSBolt_ts.class);
    private static final long serialVersionUID = -5968750340131744744L;
    ArrayDeque<TCSEvent> tcEvents;
    private double windowBoundary;
    ArrayDeque<Tuple> outWindowEvents;

    public TCSBolt_ts(int fid, SINKCombo sink){
        super(LOG,fid,sink);
    }
    public TCSBolt_ts(int fid){
        super(LOG,fid,null);
    }
...
//            LOG.info("Added event: " + event.getMyBid() + " with _bid " + _bid);
    ...
    protected void TREND_CALCULATE_REQUEST_CONSTRUCT(TCSEvent event, TxnContext txnContext) throws DatabaseException {
...
//            LOG.info("Thread " + this.thread_Id + " has reached punctuation: " + windowBoundary);
            ...
            while (!outWindowEvents.isEmpty()) {
                ...
                if (outWindowTuple.getBID() >= total_events) {//if the out-of-window events are stopping signals, directly pass to downstream
                    TREND_CALCULATE_REQUEST_POST((TCSEvent) outWindowTuple.getValue(0));
