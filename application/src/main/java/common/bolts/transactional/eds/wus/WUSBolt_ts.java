...
public class WUSBolt_ts extends WUSBolt {
    private static final Logger LOG = LoggerFactory.getLogger(WUSBolt_ts.class);
    private static final long serialVersionUID = -5968750340131744744L;
    //write-compute time pre-measured.
    ArrayDeque<WUSEvent> wuEvents;
    private double windowBoundary;
    ArrayDeque<Tuple> outWindowEvents;

    //To be used in Combo
    public WUSBolt_ts(int fid, SINKCombo sink) {
        super(LOG, fid, sink);
    }

    //To be used in ED Topology
    public WUSBolt_ts(int fid) {
        super(LOG, fid, null);
    }
    ...
    protected void PRE_TXN_PROCESS(double _bid, long timestamp) throws DatabaseException, InterruptedException {
        ...
        for (double i = _bid; i < _bid + combo_bid_size; i++) {
            ...
            WUSEvent event = (WUSEvent) input_event;
    ...
    protected void WORD_UPDATE_REQUEST_CONSTRUCT(WUSEvent event, TxnContext txnContext) throws DatabaseException {
...
//            LOG.info("Thread " + this.thread_Id + " has reached punctuation: " + windowBoundary + " with " + wuEvents.size() + " events.");
            ...
            while (!outWindowEvents.isEmpty()) {
                ...
                if (outWindowTuple.getBID() >= total_events) {//if the out-of-window events are stopping signals, directly pass to downstream
                    WORD_UPDATE_REQUEST_POST((WUSEvent) outWindowTuple.getValue(0));
