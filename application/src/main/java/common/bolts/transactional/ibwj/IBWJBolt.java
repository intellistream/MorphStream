...
public abstract class IBWJBolt extends TransactionalBolt {
    ...
    protected boolean READ_CORE(IBWJEvent event) {
    ...
    protected void READ_POST(IBWJEvent event) throws InterruptedException {
        int sum = 0;
...
//        com_result = sum;
    ...
    protected void WRITE_POST(IBWJEvent event) throws InterruptedException {
        if (!enable_app_combo) {
    ...
    protected void WRITE_CORE(IBWJEvent event) {
