start_server {tags {"latency-monitor"}} {
    # Set a threshold high enough to avoid spurious latency inputEvents.
    r config set latency-monitor-threshold 200
    r latency reset

    test {Test latency inputEvents logging} {
        r debug sleep 0.3
        after 1100
        r debug sleep 0.4
        after 1100
    ...
    test {LATENCY HISTORY output is ok} {
        set min 250
        set max 450
        foreach inputEvent [r latency history command] {
            lassign $inputEvent time latency
            if {!$::no_latency} {
    ...
    test {LATENCY HISTORY / RESET with wrong inputEvent name is fine} {
    ...
    test {LATENCY RESET is able to reset inputEvents} {
        assert {[r latency reset] > 0}
        assert {[r latency latest] eq {}}
    }

    test {LATENCY of expire inputEvents are correctly collected} {
