...
def ReadFileGS(x_axis, tthread, batchInterval, NUM_ITEMS, NUM_ACCESS, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity):
    ...
    if isCyclic == "true":
        ...
        for complexity in x_axis:
            inputEvents = tthread * batchInterval
            op_dfs_path = getPathGS("OPBFS", inputEvents, tthread, NUM_ITEMS, NUM_ACCESS, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
        ...
        for complexity in x_axis:
            inputEvents = tthread * batchInterval
            op_dfs_path = getPathGS("OPDFS", inputEvents, tthread, NUM_ITEMS, NUM_ACCESS, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
            lines = open(op_dfs_path).readlines()
            throughput = lines[0].split(": ")[1]
        ...
        for complexity in x_axis:
            inputEvents = tthread * batchInterval
            op_dfs_path = getPathGS("BFS", inputEvents, tthread, NUM_ITEMS, NUM_ACCESS, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
            lines = open(op_dfs_path).readlines()
            throughput = lines[0].split(": ")[1]
            y[1].append(float(throughput))
        ...
        for complexity in x_axis:
            inputEvents = tthread * batchInterval
            op_dfs_path = getPathGS("DFS", inputEvents, tthread, NUM_ITEMS, NUM_ACCESS, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
            lines = open(op_dfs_path).readlines()
            throughput = lines[0].split(": ")[1]
            y[2].append(float(throughput))
...
def ReadFileSL(x_axis, tthread, batchInterval, NUM_ITEMS, deposit_ratio, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity):
    ...
    if isCyclic == "true":
        for complexity in x_axis:
            inputEvents = tthread * batchInterval
            op_gs_path = getPathSL("OPGS", inputEvents, tthread, NUM_ITEMS, deposit_ratio, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
            lines = open(op_gs_path).readlines()
            throughput = lines[0].split(": ")[1]
            y[0].append(float(throughput))
        ...
        for complexity in x_axis:
            inputEvents = tthread * batchInterval
            op_dfs_path = getPathSL("OPBFS", inputEvents, tthread, NUM_ITEMS, deposit_ratio, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
            lines = open(op_dfs_path).readlines()
            throughput = lines[0].split(": ")[1]
            y[1].append(float(throughput))
        ...
        for complexity in x_axis:
            inputEvents = tthread * batchInterval
            op_dfs_path = getPathSL("OPDFS", inputEvents, tthread, NUM_ITEMS, deposit_ratio, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
            lines = open(op_dfs_path).readlines()
            throughput = lines[0].split(": ")[1]
            y[2].append(float(throughput))
    else:
        for complexity in x_axis:
            inputEvents = tthread * batchInterval
            op_gs_path = getPathSL("GS", inputEvents, tthread, NUM_ITEMS, deposit_ratio, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
            lines = open(op_gs_path).readlines()
            throughput = lines[0].split(": ")[1]
            y[0].append(float(throughput))
        ...
        for complexity in x_axis:
            inputEvents = tthread * batchInterval
            op_dfs_path = getPathSL("BFS", inputEvents, tthread, NUM_ITEMS, deposit_ratio, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
            lines = open(op_dfs_path).readlines()
            throughput = lines[0].split(": ")[1]
            y[1].append(float(throughput))
        ...
        for complexity in x_axis:
            inputEvents = tthread * batchInterval
            op_dfs_path = getPathSL("DFS", inputEvents, tthread, NUM_ITEMS, deposit_ratio, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
            lines = open(op_dfs_path).readlines()
            throughput = lines[0].split(": ")[1]
            y[2].append(float(throughput))
...
if __name__ == '__main__':
    ...
    legend = True
    y_axis = ReadFileSL(x_values, tthread, batchInterval, NUM_ITEMS, deposit_ratio, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
    DrawFigure(x_values, y_axis, legend_labels, "complexity", "throughput(e/s)", 0, 400, "sl_exploration_strategy_complexity_t{}_b{}_{}_{}_{}_{}_{}_{}_{}"
            .format(tthread, NUM_ITEMS, batchInterval, deposit_ratio, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity),
            legend)
    y_axis = ReadFileGS(x_values, tthread, batchInterval, NUM_ITEMS, NUM_ACCESS, key_skewness, overlap_ratio, abort_ratio, isCyclic, complexity)
    DrawFigure(x_values, y_axis, legend_labels, "complexity", "throughput(e/s)", 0, 400, "gs_exploration_strategy_complexity_t{}_b{}_{}_{}_{}_{}_{}_{}_{}"
