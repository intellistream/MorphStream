...
# 折线样式和颜色设置
COLOR_MAP = ['#9C1D1C','#FFAB00', '#008E42', "#772288"]
LINE_STYLES = ['-', '-', '-']
MARKERS = ['s', 'D', 'o', '^']
LINE_WIDTH = 3.0
MARKER_SIZE = 10
...
data_path = os.path.join(project_Dir, "result/data/VaryingJVMSize/stats")
FIGURE_FOLDER = os.path.join(project_Dir, "result/figures")

def generate_address_with_jvmConf(app, schedule, threads, total_events, num_items, ratio_of_deposit, state_access_skewness,
                     ratio_of_overlapped_keys, ratio_of_transaction_aborts, transaction_length, is_cyclic, complexity, jvmConf):
    # 格式化生成拼接后的地址
    address = f"{app}/{schedule}/threads = {threads}/totalEvents = {total_events}/{jvmConf}/{num_items}_{ratio_of_deposit}_{state_access_skewness}_" \
...
def DrawFigure(x_values, y_values, legend_labels, x_label, y_label, filename):
    ...
    fig.legend(
        ...
        ncol=2,  # 图例列数
        bbox_to_anchor=(0.5, 1.06),  # 控制图例位置
...
if __name__ == "__main__":
    app = "StreamLedger"
    schedule = "OG_BFS_A" # 这里添加了 schedule
    threads = 24
    ...
    throughput_results = []
    target_dir = generate_address(app, schedule, threads, total_events, num_items, ratio_of_deposit,
    ...
    variants = ["_Xms300g__Xmx300g__Xss100M__Xmn150g", "_Xms200g__Xmx200g__Xss100M__Xmn100g", "_Xms100g__Xmx100g__Xss100M__Xmn50g"]
    for variant in variants:
        target_dir = generate_address_with_jvmConf(app, schedule, threads, total_events, num_items, ratio_of_deposit,
                                      state_access_skewness,
                                      ratio_of_overlapped_keys, ratio_of_transaction_aborts, transaction_length,
                                      is_cyclic, complexity, variant)
        full_path = os.path.join(data_path, target_dir)
        print(full_path)
        throughputs = extract_phase_throughput(full_path)
        if throughputs:
    ...
    labels=["MorphSt.(w/o GC)", "MorphSt.(w/ GC - 300GB)", "MorphSt.(w/ GC - 200GB)", "MorphSt.(w/ GC - 100GB)"]
    x_values = list(range(1, 14))  # 横坐标从 1 到 13

    print(throughput_results)

    # 绘制折线图
    DrawFigure(x_values, throughput_results, labels, '', 'Throughput (K/sec)', "Figure14_a")
