...
def plot_comparison_bar_chart(x_values, y_values_1, y_values_2, labels, xlabel, ylabel, filename):
    ...
    # 设置x轴的位置
    index = np.arange(len(x_values)) * 1.5  # 增加乘以1.5来增加组与组之间的间距

    # 创建图形
    fig, ax = plt.subplots(figsize=(10, 6))

    # 绘制柱状图
    bar1 = ax.bar(index - bar_width * 1, y_values_1, bar_width, label=labels[0], color=COLOR_MAP[0])
    bar2 = ax.bar(index + bar_width * 1, y_values_2, bar_width, label=labels[1], color=COLOR_MAP[1])
...
if __name__ == "__main__":
    ...
    for variant in variants:
        target_dir = generate_address(app, "OG_NS_A", threads, total_events, num_items, ratio_of_deposit,
                                      state_access_skewness,
                                      ratio_of_overlapped_keys, ratio_of_transaction_aborts, transaction_length,
                                      variant, complexity)
        full_path = os.path.join(data_path, target_dir)
    ...
    x_values = ["Cyclic", "Acyclic"]  # x 轴值

# 示例数据
labels = ["f-schedule", "c-schedule"]  # 图例标签

plot_comparison_bar_chart(x_values, throughput_results_1, throughput_results_2, labels, "", "Throughput (k/sec)", "Figure16_a")
