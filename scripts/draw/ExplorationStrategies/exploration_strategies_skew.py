...
COLOR_MAP = ['#FF6300', '#005F5C','#005F5C']  # 橘色和深绿色
LINE_STYLES = ['-', '--', '-.']
...
data_path = os.path.join(project_Dir, "result/data/ExplorationStrategies/stats")
...
def plot_comparison_bar_chart(x_values, y_values_1, y_values_2, y_values_3, labels, xlabel, ylabel, filename):
    """
    绘制对比柱状图，支持三个柱子，带不同填充样式。
    """
    # 设置柱状图的宽度
    bar_width = 0.25
    ...
    # 填充样式
    hatch_patterns = ['/', '\\', 'x']  # 分别为右划线、左划线、小方格

    # 绘制柱状图
    bar1 = ax.bar(index - bar_width, y_values_1, bar_width, label=labels[0], color=COLOR_MAP[0], hatch=hatch_patterns[0])
    bar2 = ax.bar(index, y_values_2, bar_width, label=labels[1], color=COLOR_MAP[1], hatch=hatch_patterns[1])
    bar3 = ax.bar(index + bar_width, y_values_3, bar_width, label=labels[2], color=COLOR_MAP[2], hatch=hatch_patterns[2])

    # 设置x轴标签和y轴标签
    ax.set_xlabel(xlabel, fontproperties=LABEL_FP, fontsize=LABEL_FONT_SIZE)
    ax.set_ylabel(ylabel, fontproperties=LABEL_FP, fontsize=LABEL_FONT_SIZE)
...
if __name__ == "__main__":
    ...
    for variant in variants:
        target_dir = generate_address(app, "OG_NS", threads, total_events, num_items, ratio_of_deposit,
                                      variant,
    ...
    for variant in variants:
        target_dir = generate_address(app, "OG_BFS", threads, total_events, num_items, ratio_of_deposit,
                                      variant,
                                      ratio_of_overlapped_keys, ratio_of_transaction_aborts, transaction_length,
                                      is_cyclic, complexity)
        full_path = os.path.join(data_path, target_dir)
    ...
    throughput_results_3 = []
    for variant in variants:
        target_dir = generate_address(app, "OG_DFS", threads, total_events, num_items, ratio_of_deposit,
                                      variant,
                                      ratio_of_overlapped_keys, ratio_of_transaction_aborts, transaction_length,
                                      is_cyclic, complexity)
        full_path = os.path.join(data_path, target_dir)
        print(full_path)
        throughput_results = extract_throughput(full_path)
        throughput_results_3.append(throughput_results)
    print(throughput_results_3)

    x_values = [0,0.25,0.5,0.75,1]  # x 轴值

# 示例数据
labels = ["ns-explore", "s-explore (BFS)", "s-explore (DFS)"]  # 图例标签

plot_comparison_bar_chart(x_values, throughput_results_1, throughput_results_2, throughput_results_3, labels, "Zip skew factor", "Throughput (k/sec)", "Figure15_b")
