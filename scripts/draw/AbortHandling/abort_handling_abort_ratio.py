...
data_path = os.path.join(project_Dir, "result/data/AbortHandling/stats")
...
def plot_comparison_bar_chart(x_values, y_values_1, y_values_2, labels, xlabel, ylabel, filename):
    ...
    # 绘制柱状图
    bar1 = ax.bar(index - bar_width / 2, y_values_1, bar_width, label=labels[0], color=COLOR_MAP[0])
    bar2 = ax.bar(index + bar_width / 2, y_values_2, bar_width, label=labels[1], color=COLOR_MAP[1])
...
if __name__ == "__main__":
    ...
    variants = [1000, 3000, 5000, 7000, 9000]
    throughput_results_1 = []
    for variant in variants:
        target_dir = generate_address(app, "OP_NS_A", threads, total_events, num_items, ratio_of_deposit,
                                      state_access_skewness,
                                      ratio_of_overlapped_keys, variant, transaction_length,
    ...
    for variant in variants:
        target_dir = generate_address(app, "OP_NS", threads, total_events, num_items, ratio_of_deposit,
                                      state_access_skewness,
                                      ratio_of_overlapped_keys, variant, transaction_length,
                                      is_cyclic, complexity)
        full_path = os.path.join(data_path, target_dir)
    ...
    x_values = [10, 30, 50, 70, 90]  # x 轴值

# 示例数据
labels = ["e-abort", "l-abort"]  # 图例标签

plot_comparison_bar_chart(x_values, throughput_results_1, throughput_results_2, labels, "Transaction Abort Ratio (%)", "Throughput (k/sec)", "Figure17_b")
