...
public class NGSTxnEvent extends TxnEvent {
    ...
    public NGSTxnEvent(long bid, int pid, String bid_array, String partition_index, int num_of_partition, String key_array, int TOTAL_NUM_ACCESS, int Transaction_Length, boolean isAbort, boolean isNon_Deterministic_StateAccess) {
    ...
    public NGSTxnEvent cloneEvent() {
        return new NGSTxnEvent(bid, pid, Arrays.toString(bid_array), Arrays.toString(partition_indexs), number_of_partitions, Arrays.toString(keys), TOTAL_NUM_ACCESS, Txn_Length, isAbort, isNon_Deterministic_StateAccess);
    }
}