...
public abstract class SHJBolt extends TransactionalBolt {
    ...
    protected void SHJ_LOCK_AHEAD(SHJTxnEvent event, TxnContext txnContext) throws DatabaseException {
...
//        transactionManager.lock_ahead(txnContext, "index_s_table", event.getKey(), event.tarIndexRecordRef, READ_WRITE);
    ...
    protected void SHJ_REQUEST_NOLOCK(SHJTxnEvent event, TxnContext txnContext) throws DatabaseException { //TODO: Follow GS, rewrite this section. Consider remove key.
        String updateIndexTable = ""; //index to update
        String lookupIndexTable = ""; //index to lookup
        if (Objects.equals(event.getStreamID(), "r")) {
            updateIndexTable = "index_r_table";
...
//        assert event.srcIndexRecordRef.getRecord() != null || event.tarIndexRecordRef.getRecord() != null;
    ...
    protected void SHJ_REQUEST_CORE(SHJTxnEvent event) {
    ...
    protected void POST_PROCESS(double _bid, long timestamp, int combo_bid_size) throws InterruptedException {
        ...
        for (double i = _bid; i < _bid + combo_bid_size; i++) {
            ((SHJTxnEvent) input_event).setTimestamp(timestamp);
            SHJ_POST((SHJTxnEvent) input_event);
        }
        END_POST_TIME_MEASURE(thread_Id);
    }

    protected boolean SHJ_CORE(SHJTxnEvent event) { //TODO: tstream
    ...
    protected void SHJ_POST(SHJTxnEvent event) throws InterruptedException { //TODO: tstream
