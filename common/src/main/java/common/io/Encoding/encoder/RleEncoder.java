...
public abstract class RleEncoder<T extends Comparable<T>> extends Encoder{
    ...
    public void flush(ByteArrayOutputStream out) throws IOException {
        ...
        if (repeatCount >= FileConfig.RLE_MIN_REPEATED_NUM) {
            try {
                writeRleRun();
            } catch (IOException e) {
                logger.error(
    ...
    public void writeOrAppendBitPackedRun() {
        if (bitPackedGroupCount >= FileConfig.RLE_MAX_BIT_PACKED_NUM) {
    ...
    protected void encodeValue(T value) {
        ...
        if (value.equals(preValue)) {
            repeatCount++;
            if (repeatCount >= FileConfig.RLE_MIN_REPEATED_NUM
                    && repeatCount <= FileConfig.RLE_MAX_REPEATED_NUM) {
                ...
                return;
            } else if (repeatCount == FileConfig.RLE_MAX_REPEATED_NUM + 1) {
                ...
                try {
                    ...
                    logger.error(
                            " error occurs when writing full rle run to OutputStram when repeatCount = {}."
                                    + "numBufferedValues {}, repeatCount {}, bitPackedGroupCount{}, "
                                    + "isBitPackRun {}, isBitWidthSaved {}",
                            FileConfig.RLE_MAX_REPEATED_NUM + 1,
                            numBufferedValues,
                            repeatCount,
                            bitPackedGroupCount,
                            isBitPackRun,
                ...
                try {
                    ...
                    logger.error(
                            "tsfile-encoding RleEncoder : error occurs when writing num to OutputStram "
                                    ...
                                    + "isBitWidthSaved {}",
                            FileConfig.RLE_MIN_REPEATED_NUM,
                            numBufferedValues,
                            repeatCount,
                            bitPackedGroupCount,
                            isBitPackRun,
        ...
        if (numBufferedValues == FileConfig.RLE_MIN_REPEATED_NUM) {
            writeOrAppendBitPackedRun();
        }
    }
    ...
    public void encode(boolean value, ByteArrayOutputStream out) {
        throw new EncodingException(getClass().getName());
    }
