...
public class RdmaMappedFile {
    private static final Logger LOG = LoggerFactory.getLogger(RdmaMappedFile.class);
    ...
    private void mapAndRegister(int chunkSize, long[] partitionLengths) throws IOException, InvocationTargetException, IllegalAccessException {
        long offset = 0;
        long curLength = 0;
    ...
    public ByteBuffer getByteBufferForPartition(int partitionId) throws IOException {
        ...
        return (rdmaBlockLocation.length == 0) ? null : getByteBuffer(rdmaBlockLocation.address, rdmaBlockLocation.length);
    }
