...
private[spark] class RdmaShuffleManager(val conf: SparkConf, isDriver: Boolean) extends ShuffleManager with Logging {
  ...
  private val rdmaShuffleManagersMap = new ConcurrentHashMap[RdmaShuffleManagerId, RdmaChannel]().asScala
  val blockManagerIdToRdmaShuffleManagerId = new ConcurrentHashMap[BlockManagerId, RdmaShuffleManagerId]().asScala
  ...
  val shuffleIdToMapAddressBuffer = new ConcurrentHashMap[ShuffleId, Future[RdmaBuffer]]()
  ...
  private def getRdmaChannel(host: String, port: Int, mustRetry: Boolean, rdmaChannelType: RdmaChannel.RdmaChannelType): RdmaChannel = rdmaNode.get.getRdmaChannel(new InetSocketAddress(host, port), mustRetry, rdmaChannelType)

  def getRdmaChannel(rdmaShuffleManagerId: RdmaShuffleManagerId, mustRetry: Boolean): RdmaChannel = { getRdmaChannel(rdmaShuffleManagerId.host, rdmaShuffleManagerId.port, mustRetry,
     RdmaChannel.RdmaChannelType.RDMA_READ_REQUESTOR)
  }

  def getRdmaChannelToDriver(mustRetry: Boolean): RdmaChannel = getRdmaChannel(rdmaShuffleConf.driverHost, rdmaShuffleConf.driverPort, mustRetry, RdmaChannel.RdmaChannelType.RPC)

  def getRdmaBufferManager: RdmaBufferManager = rdmaNode.get.getRdmaBufferManager

  def getRdmaByteBufferManagedBuffer(length : Int): RdmaByteBufferManagedBuffer = new RdmaByteBufferManagedBuffer(new RdmaRegisteredBuffer(getRdmaBufferManager, length), length)

  def getRdmaRegisteredBuffer(length : Int): RdmaRegisteredBuffer = new RdmaRegisteredBuffer(getRdmaBufferManager, length)
   ...
   * @param mapTaskOutput
   */
  def publishMapTaskOutput(shuffleId: ShuffleId, mapId: MapId, mapTaskOutput: RdmaMapTaskOutput): Future[Boolean] = {
    assume(!isDriver)
