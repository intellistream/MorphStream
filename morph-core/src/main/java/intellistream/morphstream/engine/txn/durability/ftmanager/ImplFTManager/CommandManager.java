 ...
 * Command Manager
 */
public class CommandManager extends FTManager {
    ...
    private String commandLogMetaPath;
    ...
    public void initialize(Configuration config) throws IOException {
        ...
        commandLogPath = config.getString("rootFilePath") + OsUtils.OS_wrapper("logging");
        commandLogMetaPath = config.getString("rootFilePath") + OsUtils.OS_wrapper("logging") + OsUtils.OS_wrapper("metaData.log");
        isRecovery = config.getBoolean("isRecovery");
        isFailure = config.getBoolean("isFailure");
        File commandLogFile = new File(commandLogPath);
        if (!commandLogFile.exists()) {
            commandLogFile.mkdirs();
        }
        if (isRecovery) {
            RecoveryHelperProvider.getCommittedLogMetaData(new File(commandLogMetaPath), LoggingCommitInformation);
        }
        LOG.info("CommandManager initialize successfully");
    ...
    public void Listener() throws IOException {
        while (running) {
            if (all_register()) {
                LOG.info("CommandManager received all register and commit log");
    ...
    public void run() {
        LOG.info("CommandManager starts!");
        try {
            Listener();
        } catch (IOException e) {
            ...
            if (!isFailure) {
                File file = new File(this.commandLogPath);
                FileSystem.deleteFile(file);
            }
            LOG.info("CommandManager stops");
        }
    }
    ...
    private void logComplete(long pendingId) throws IOException {
        ...
        LocalDataOutputStream localDataOutputStream = new LocalDataOutputStream(new File(this.commandLogMetaPath));
        ...
        LOG.info("CommandManager commit the commandLog to the current.log");
    }
}
