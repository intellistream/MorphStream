package intellistream.morphstream.engine.db.storage.store.disk;
...
public class Page {
    ...
    public Page(FileChannel fc, int blockNum, int pageNum, boolean durable) {
        ...
        try {
            this.pageData = fc.map(FileChannel.MapMode.READ_WRITE, (long) blockNum * Page.pageSize, Page.pageSize);
        } catch (IOException e) {
