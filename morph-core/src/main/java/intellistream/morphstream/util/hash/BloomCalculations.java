package intellistream.morphstream.util.hash;
...
public class BloomCalculations {
    ...
    /**
     * In the following table, the SchemaRecord 'i' shows false positive rates if i buckets
     * per element are used.  Column 'j' shows false positive rates if j hash
     * functions are used.  The first SchemaRecord is 'i=0', the first column is 'j=0'.
     * Each cell (i,j) the false positive rate determined by using i buckets per
     * element and j hash functions.
     */
    private static final double[][] probs = new double[][]{
            {1.0}, // dummy SchemaRecord representing 0 buckets per element
            {1.0, 1.0}, // dummy SchemaRecord representing 1 buckets per element
            {1.0, 0.393, 0.400},
