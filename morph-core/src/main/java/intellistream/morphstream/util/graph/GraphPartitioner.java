...
public class GraphPartitioner {
    private final int nodeSize;  // Node
    private final List<Edge> edges;  // Edge
    private final int[][] partitionEdgesWeight;// Edge weight between partitions
    private final int[] partitionWeights;  // Partition weight
    private final int[][] benefits;  // the benefit to move node_i to partition_j int[i][j] Vi -> Pj
    private final int partitionCount;  // Partition count
    ...
    public static int assignTasksByDP(int[] tasks, int n, int[][] dp, int[][] plan) {
        ...
        if (tasks.length - 1 >= 0) System.arraycopy(sum, 2, dp[1], 2, tasks.length - 1);
