...
public interface FileInput {
     ...
     * @throws IOException If some other I/O error occurs
     */
    FileInput position(long newPosition) throws IOException;

    /**
     * Reads a sequence of bytes from this FileInput into the given buffer.
     *
     * <p>Bytes are read starting at this FileInput's current position, and then the position is
    ...
    /**
     * Reads a sequence of bytes from this FileInput into the given buffer, starting at the given
     * position.
     *
     * <p>This method works in the same manner as the {@link #read(ByteBuffer)} method, except that
     * bytes are read starting at the given position rather than at the FileInput's current
     * position. This method does not modify this FileInput's position. If the given position is
     * greater than the FileInput's current size then no bytes are read.
     *
     ...
     * @throws ClosedChannelException If this FileInput is closed
     * @throws AsynchronousCloseException If another thread closes this FileInput while the read
