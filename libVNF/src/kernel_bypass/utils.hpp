...
struct Globals {
  ...
  //  unordered_map<int, string> socketProtocolMap;
    ...
    Globals() : onAcceptByServerCallback(),
                onAcceptByServerDSCallback(nullptr),
                onAcceptByServerReqObjIdExtractor(nullptr),
                onAcceptByServerPBD(nullptr),
...
int readFromStream(mctx_t mctx, int connFd, uint8_t *buf, int len) {
    if (connFd < 0 || len <= 0) {
        return -1;
    }
    ...
    int ptr = 0;
    while (true) {
        int readBytes = static_cast<int>(mtcp_read(mctx, connFd, buf + ptr, remainingBytes));
        if (readBytes <= 0) {
            return readBytes;
        }
        ptr += readBytes;
