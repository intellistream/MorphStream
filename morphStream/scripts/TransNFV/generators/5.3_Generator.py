...
def generate_workload_with_keySkew(exp_dir, expID, vnfID, numPackets, numInstances, numItems, keySkew, workloadSkew, readRatio,
                                   locality, scopeRatio, puncInterval):
...
def generate_workload_with_locality(exp_dir, expID, vnfID, numPackets, numInstances, numItems, keySkew, workloadSkew, readRatio,
                                    locality, scopeRatio, puncInterval):
    rootDir = f'{exp_dir}/workload'
    workloadConfig = f'temporary/{expID}/vnfID={vnfID}/numPackets={numPackets}/numInstances={numInstances}/numItems={numItems}/keySkew={keySkew}/workloadSkew={workloadSkew}/readRatio={readRatio}/locality={locality}/scopeRatio={scopeRatio}'
    workloadDir = f'{rootDir}/{workloadConfig}'
...
def combine_csv_randomly(combinedWorkloadPath, exp_dir):
    ...
    for workloadInterval in workloadIntervalList:
        ...
        for instanceID in range(numInstances):
            ...
            for phaseID in range(num_phases):
                expID = f'5.3_Interval{workloadInterval}_Phase{phaseID}'
                workloadFilePath = ''

                if phaseID % 2 == 0:
                    workloadFilePath = f'{exp_dir}/temporary/{expID}/vnfID={vnfID}/numPackets={workloadInterval}/' \
...
Offloading_keySkew = 0
Offloading_workloadSkew = 0
Offloading_readRatio = 0
Offloading_locality = 75
...
def generate(exp_dir):
    ...
    for workloadInterval in workloadIntervalList:
        ...
        for phase in range(num_phases):
            if (phase % 2 == 0):
                expID = f'5.3_Interval{workloadInterval}_Phase{phase}'
                generate_workload_with_locality(exp_dir, expID, vnfID, workloadInterval, numInstances, numItems,
                                                Partition_keySkew,
                                                Partition_workloadSkew,
                                                Partition_readRatio, Partition_locality, Partition_scopeRatio,
                                                puncInterval)
            else:
                expID = f'5.3_Interval{workloadInterval}_Phase{phase}'
                generate_workload_with_locality(exp_dir, expID, vnfID, workloadInterval, numInstances, numItems,
                                                Offloading_keySkew,
                                                Offloading_workloadSkew,
                                                Offloading_readRatio, Offloading_locality, Offloading_scopeRatio,
                                                puncInterval)
